...
final class Menu_Icons_Meta {
	 ...
	 *
	 * @since   0.3.0
	 * @mn_hook filter is_protected_meta
	...
	public static function get( $id, $defaults = array() ) {
		$defaults = mn_parse_args( $defaults, self::$defaults );
		$value    = get_post_meta( $id, self::KEY, true );
		$value    = mn_parse_args( (array) $value, $defaults );
		...
		if ( ! empty( $value['icon'] ) && in_array( $value['type'], array( 'image', 'svg' ) ) ) {
			$value['url'] = mn_get_attachment_image_url( $value['icon'], 'thumbnail', false );
		}
