...
abstract class Icon_Picker_Type_Font extends Icon_Picker_Type {
	 ...
	 *
	 * @since   0.1.0
	 * @mn_hook action icon_picker_loader_init
	 *
	...
	public function register_assets( Icon_Picker_Loader $loader ) {
		...
		if ( $styles->query( $this->stylesheet_id, 'registered' ) ) {
			...
			if ( version_compare( $object->ver, $this->version, '<' ) ) {
				$deps = $object->deps;
				mn_deregister_style( $this->stylesheet_id );
			} else {
				$register = false;
			}
		}
		...
		if ( $register ) {
			mn_register_style( $this->stylesheet_id, $this->stylesheet_uri, $deps, $this->version );
		}
