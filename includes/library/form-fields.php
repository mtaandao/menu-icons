...
abstract class Kucrut_Form_Field {
	...
	final public static function create( Array $field, $args = array() ) {
		$field = mn_parse_args( $field, self::$defaults['field'] );
		...
		$args  = (object) mn_parse_args( $args, self::$defaults['args'] );
	...
	public function description() {
		if ( ! empty( $this->field['description'] ) ) {
			...
			printf(
				...
				$tag, // xss ok
				mn_kses( $this->field['description'], $this->allowed_html )
			);
		}
	}
}
...
class Kucrut_Form_Field_Select_Pages extends Kucrut_Form_Field_Select {
	...
	protected $mn_dropdown_pages_args = array(
		'depth'             => 0,
		'child_of'          => 0,
		'option_none_value' => '',
	);
	...
	public function __construct( $field, $args ) {
		$this->mn_dropdown_pages_args['show_option_none'] = __( '&mdash; Select &mdash;', 'postmedia' );
	...
	public function set_properties() {
		...
		if ( empty( $this->args->mn_dropdown_pages_args ) ) {
			$this->args->mn_dropdown_pages_args = array();
		}

		// Apply defeaults
		$this->args->mn_dropdown_pages_args = mn_parse_args(
			$this->args->mn_dropdown_pages_args,
			$this->mn_dropdown_pages_args
		);

		// Force some args
		$this->args->mn_dropdown_pages_args = array_merge(
			$this->args->mn_dropdown_pages_args,
			array(
				'echo'     => true,
	...
	public function render() {
		mn_dropdown_pages( $this->args->mn_dropdown_pages_args );
	}
}
